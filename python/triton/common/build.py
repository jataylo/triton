...
def _build(name, src, srcdir):
    ...
    if is_hip():
        ret = subprocess.check_call([cc, src, f"-I{hip_include_dir}", f"-I{py_include_dir}", f"-I{srcdir}", "-shared", "-fPIC", f"-L{hip_lib_dir}", f"{hip_lib_dir}/lamdhip64.so.5", f"-Wl,-rpath={hip_lib_dir}", "-o", so])
    else:
